...
function preload() {
  ...
  bgMusic  = loadSound("assets/sounds/203_SQ3_Music.mp3");
  hitBangSound = loadSound("assets/sounds/302_hit_bang.mp3");

  bgImage = loadImage("assets/images/203_SQ3_Background.webp");
  victoryBG = loadImage("assets/images/302_SQ3_VictoryBG.jpg");
  startBG = loadImage("assets/images/203_SQ3_StartBG.jpg");

  hitBG1 = loadImage("assets/images/203_SQ3_Hit_BG1.webp");
hitBG2 = loadImage("assets/images/203_SQ3_Hit_BG2.png");
}
...
function draw() {
  ...
  if (screenFlash > 0) {
  fill(255, 255, 255, screenFlash * 20);  // fade out
...
function drawStartScreen() {
  ...
  textAlign(CENTER);
  textSize(52);
  text("DIXIT FIGHT CITY", width/2, height/2 - 60);

  fill(255); // white for the prompt
  textSize(18);
...
function drawWinScreen() {
  image(victoryBG, 0, 0, width, height);
...
function drawArena() {
  if (totalHits === 0) {
    ...
    image(hitBG1, 0, 0, width, height);
  } else if (totalHits === 2) {
    image(hitBG2, 0, 0, width, height);
  }
}
...
function checkHits() {
  ...
  if (fighter1.isAttacking && !fighter1.hitLanded) {
    ...
    if (dist < fighter2.r + 10) {
      ...
      totalHits++;
      screenFlash = 8;
      hitBangSound.play();   // plays when hit connects
    }
  }
